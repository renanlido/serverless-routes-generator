...
program.command("generate").description("Generate serverless routes").action(() => {
  ...
  (0, import_child_process.exec)(`npx tsx ${scriptPath}`, (error, stdout, stderr) => {
    if (error) {
...
program.command("init").description("Cria um arquivo de configura\xE7\xE3o padr\xE3o no diret\xF3rio raiz").action(() => {
  ...
  const configFilePath = import_path.default.resolve(process.cwd(), configFileName);
  if (import_fs.default.existsSync(configFilePath)) {
    console.error(`O arquivo "${configFileName}" j\xE1 existe no diret\xF3rio raiz.`);
    process.exit(1);
  }
  import_fs.default.writeFile(configFilePath, configContent, "utf8", (err) => {
    if (err) {
