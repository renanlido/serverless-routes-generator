...
var lambdaIsRunning = /* @__PURE__ */ __name(() => {
  if (process.env["AWS_LAMBDA_FUNCTION_NAME"] || process.env["AZURE_FUNCTIONS_ENVIRONMENT"] || process.env["GOOGLE_CLOUD_FUNCTION_NAME"]) {
    return true;
  }
  return false;
}, "lambdaIsRunning");
...
function getContext(match) {
  if (match && match[1]) {
    ...
    const directory = import_path.default.dirname(fullPath);
    const indexSRC = directory.indexOf(import_path.default.join("src"));
    if (indexSRC !== -1) {
...
var createHandler = /* @__PURE__ */ __name((config, handler) => {
  ...
  const matchPath = new Error().stack.split("\n")[2].match(/\(([^:]+):\d+:\d+\)/);
  if (!matchPath) {
...
var _ServerlessGenerator = class _ServerlessGenerator {
  ...
  generateServerlessConfig() {
    ...
    routes.forEach((route) => {
      ...
      const handlerPath = path2.relative(this.basePath, path2.join(this.basePath, route.context, route.handler));
...
async function importAllHandlers(data) {
  const handlersPath = path3.join(process.cwd(), `${data.projectRoot.concat("/").concat(data.pathPattern)}`);
...
async function importConfigFile() {
  try {
    ...
    throw new Error(`Could not find config file at ${configFileJson} or ${configFileJs}`);
  } catch (error) {
    const err = error;
